...
def main():
    ...
    # Create path names
    l1a_prod_version = os.path.basename(runconfig["raw_img_path"]).split(".")[0].split("_")[-1]
    l1b_prod_version = f"v{runconfig['product_version']}"
    rdn_basename = os.path.basename(runconfig["raw_img_path"]).replace("l1a", "l1b").replace("raw", "rdn").replace(l1a_prod_version, l1b_prod_version)
