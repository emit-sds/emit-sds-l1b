...
def err(f, fpa, frames, ghost_config, coarse):
    siz = f.size()
    x = np.array([f.get_coord(i) for i in range(siz)])
    sum_errs = err_npy(x, fpa, frames, ghost_config, coarse)
    f.setBBO(str(sum_errs).encode("UTF-8"))
    return 1

def err_npy(x, fpa, frames, ghost_config, coarse):
...
def main():
    ...
    for coarse in [2,0,2]:#,1,0,2,1,0,2]:
        ...
        params = ["BB_OUTPUT_TYPE OBJ", "MAX_BB_EVAL 100", "DISPLAY_DEGREE 2", "DISPLAY_ALL_EVAL true", "DISPLAY_STATS BBE OBJ"] 
        lb = [b[0] for b in bounds]
        ub = [b[1] for b in bounds]

        result = PyNomad.optimize(lambda f: err(f, fpa, frames, ghost_config, coarse), x0, lb, ub, params)

        fmt = ["{} = {}".format(n,v) for (n,v) in result.items()]
        output = "\n".join(fmt)
        print("\nNOMAD results \n" + output + " \n")

        best_config = deserialize_ghost_config(np.array(result['x_best']), ghost_config, \
            coarse=coarse)
